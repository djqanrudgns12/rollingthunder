...
import { Settings2, Map as MapIcon, Maximize2 } from 'lucide-react'
  ...
  return (
    ...
    >
      <div className="flex-1 p-4 space-y-4">
        ...
        <p className="text-gray-500 text-xs mt-2 text-center border-t border-[#333] pt-3">기물을 선택하면 개별 속성이 표시됩니다.</p>
      </div>
    </FloatingPanel>
  )
}

function ItemProperties({ selectedItem }: { selectedItem: EditorItem }) {
  const { updateItem, setSelectedItemId } = useEditorStore()
...
export default function InspectorPanel() {
  ...
  return (
    <>
      <StageProperties />
      {selectedItem && <ItemProperties selectedItem={selectedItem} />}
    </>
  )
}
