...
export default async function AdminUsersPage() {
  // getUsers()가 이미 { count: 'exact' }로 총 수를 반환하므로 별도 count 쿼리 불필요
  const { data: initialUsers, count } = await getUsers(1, 50)
  ...
  return (
    <>
      ...
      <UserTable initialUsers={initialUsers || []} initialCount={count || 0} />
    </>
  )
}
