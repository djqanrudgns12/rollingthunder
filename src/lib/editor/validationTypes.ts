...
export const VALIDATION_THRESHOLDS = {
  finishTime: { min: 10, max: 180 },   // 완주시간 중앙값(초)
  fairness: { max: 0.6 },              // |pearson(스폰x, 순위)|
  edgeHugging: { maxPct: 50 },         // 엣지허깅 칩 비율(%)
  leadChanges: { min: 1 },             // 레이스당 선두 교체 평균
  deadGimmickRate: 0.03,               // 적중률 이 값 미만이면 "죽은 기믹"
  stall: { maxStorms: 10, maxTimedOutRaces: 2 }, // 중력폭풍/미완주 레이스 허용치
} as const

/** 검증 패널 지표 라벨/설명 카피. checks[i].label 을 키로 매칭. */
export const METRIC_INFO: Record<string, { tooltip: string }> = {
  '완주시간(중앙)': { tooltip: '칩 절반이 완주하는 데 걸리는 시간. 10~180초 범위면 통과합니다. 짧으면 기물을 늘리거나 맵을 연장하고, 길면 병목을 제거하세요.' },
  '공정성': { tooltip: '출발 위치(가로)와 최종 순위의 상관관계. 0.6 이상이면 특정 출발 자리가 크게 유리한 맵입니다. 좌우 대칭 배치로 완화하세요.' },
  '엣지허깅': { tooltip: '경기의 절반 이상을 측벽에 붙어 내려간 칩 비율. 50% 이상이면 벽 쪽이 지름길입니다 — 벽 근처에 핀/범퍼를 배치하세요.' },
  '정체(스톰/미완주)': { tooltip: '중력폭풍(장기 정체 구제) 발동 횟수 / 제한시간(4분) 내 미완주 레이스 수. 허용치를 크게 넘으면 칩이 갇히는 지형이 있습니다 — 히트맵의 빨간 지점을 확인하세요.' },
  '박진감(선두교체)': { tooltip: '레이스당 1위가 바뀐 평균 횟수. 1회 미만이면 초반에 승부가 굳는 단조로운 맵입니다.' },
  '기믹적중': { tooltip: '각 기믹(풍차·부스터·포탈 등)을 통과한 칩 비율. 3% 미만인 "죽은 기믹"은 위치를 조정하거나 제거하세요.' },
}
