...
### 🗺️ Phase 6: 인게임 맵 에디터 (Developer Tool)
**목표**: 개발자(어드민)가 직접 인게임에서 물리 맵과 장애물을 배치하고, 즉시 Supabase에 동적으로 배포할 수 있는 자체 에디터 구축.
- **[6.1] 관리자 인증 기반 마련**: 기존 Supabase Auth를 활용하여 기본 회원가입/로그인을 먼저 구현(`Phase 2`의 인증 파트 우선 적용). 개발자 본인 계정에만 `admin` Role을 부여하여 에디터 접근 통제. (참고: 일반 유저용 회원가입이 당장 필요 없더라도, Supabase RLS로 안전하게 에디터를 보호하려면 '최소한의 관리자 인증' 구현은 필수적입니다.)
- **[6.2] DB 기반 맵 로딩 시스템 전환**: 현재 `MapPresets.ts`에 하드코딩된 정적(Static) 데이터를 Supabase `maps` 테이블에서 동적(Dynamic)으로 Fetch 해오는 방식으로 엔진 구조 수정.
  - **장기적 안정성(오류) 고려**: 통신 지연 시 '로딩 중' 화면 처리, 네트워크 단절이나 DB 오류 시 `MapPresets.ts`의 로컬 데이터를 불러와 게임 진행이 멈추지 않게 하는 **Fallback(안전망)** 메커니즘 필수 구현.
- **[6.3] 에디터 모드 UI 및 인터랙션**: Zustand에 `EDIT` 모드를 분리하고, Pixi.js 객체의 드래그 앤 드롭 및 Rapier 물리 골격 렌더링(Debug Draw) 활성화. 정밀 좌표 수정을 위한 프로퍼티 패널 UI 구현.
- **[6.4] DB 저장 (동적 배포)**: 에디터에서 [저장] 클릭 시, 구성된 맵을 JSON 배열로 압축하여 Supabase에 Upsert.
- **체크포인트**: 에디터에서 기물의 X, Y 좌표를 수정한 후 저장하고 새로고침 했을 때, 게임에 딜레이 없이 즉각 반영되는지 확인.
