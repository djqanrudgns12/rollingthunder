...
## 💎 상점 경제(Economy) 및 프리미엄 스킨 확장 모델

> **프리미엄 에셋 기반의 칩 소모(Sink) 시스템 다각화**

### 1. 스킨 및 아바타 등급별 가격 정책 (Pricing Policy)
인게임 재화(칩) 밸런스를 맞추기 위해 스킨과 아바타의 가격을 등급별로 명확히 구조화합니다.
...
* **Skins (참가자 스킨)**
  ...
  * Epic: 5,000 칩
  * Legendary: 10,000 칩
  * Mythic: 15,000 칩 (최상위 등급)
* **Avatars (프로필 아바타)**
  * Rare: 1,000 칩 (기본 지급 제외)
  * Epic: 3,000 칩
  * Legendary: 7,000 칩 (관리자 전용 제외)
  * Mythic: 10,000 칩

### 2. 프리미엄 스킨 12종 확장
게임 내 수익 모델 및 유저 목표(Goal) 제공을 위해 최고급 품질의 PNG 스킨 12종을 추가합니다. 모든 스킨은 2D 게임 에셋 형태(투명 배경, 아이소메트릭/탑다운)를 유지합니다.

* **Rare (3종)**: 마법 물약, 종이 비행기, 고무 오리
* **Epic (3종)**: 메카 장수풍뎅이, 호버보드, 크리스탈 골렘
* **Legendary (1종)**: 레비아탄 (화려한 원소 이펙트 동반)
* **Mythic (5종)**: 우주 천사, 심연의 군주, 은하 핵, 사이버 데몬, 시간의 직조자 (`masterpiece`, `hyper-detailed` 강제 적용)
