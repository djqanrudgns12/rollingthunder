...
ADD COLUMN IF NOT EXISTS chips_balance BIGINT DEFAULT 0,
...
BEGIN
    ...
    -- Row-level lock 획득
    SELECT chips_balance INTO v_current_chips
    ...
    SET chips_balance = chips_balance + p_amount,
        ...
        updated_at = NOW()
    WHERE id = p_user_id
    RETURNING chips_balance INTO v_current_chips;
...
BEGIN
    ...
    IF v_current_chips < p_amount THEN
        RAISE EXCEPTION 'Insufficient chips_balance. Has: %, Needs: %', v_current_chips, p_amount;
    END IF;
